...
import useStyles from './SelectComplaintStyles';
...
const SelectComplaints = () => {
  ...
  const goToSelectSlot = () => {
    ...
    if (selectedAddress) {
      getDoctorFromPostalCode(selectedAddress.postalCode)
        .then((response: { data: any[] }) => {
          if (response.data[0]) {
            localStorage.setItem('selected-doctor', JSON.stringify(response.data[0]));
...
export default SelectComplaints;
